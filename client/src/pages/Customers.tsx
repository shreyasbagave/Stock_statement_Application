...
const CustomersPage: React.FC = () => {
    ...
    const load = async () => {
        ...
        setError(null);
        try {
            const res = await api.get<CustomersResponse>(`/customers?${query}`);
            if (res.data?.success) {
    ...
    const submit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (editItem) {
                await api.post(`/customers/${editItem._id}?_method=PUT`, payload);
            } else {
    ...
    const remove = async (id: string) => {
        ...
        try {
            await api.post(`/customers/${id}?_method=DELETE`);
            await load();
        } catch (err: any) {
    ...
    return (
        <div style={{ padding: 24 }}>
            ...
            <div className="filter-bar" style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
                ...
                <button onClick={openCreate} style={{ marginLeft: 'auto' }}>Add Customer</button>
            </div>
            {loading ? (
                <div>Loading...</div>
            ) : error ? (
                ...
                <div style={{ overflowX: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                        ...
                        <tbody>
                            ...
                            {items.length === 0 && (
                                <tr><td colSpan={3} style={{ padding: 12, textAlign: 'center' }}>No customers</td></tr>
                            )}
                        </tbody>
                    </table>
            ...
            {formOpen && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.35)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <div style={{ background: '#fff', padding: 16, borderRadius: 8, width: 'min(100% - 24px, 520px)' }}>
                        <h3 style={{ marginBottom: 12 }}>{editItem ? 'Edit Customer' : 'Add Customer'}</h3>
